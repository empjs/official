...
`pluginRspackEmpShare` 是新增的EMP插件，主要用来配置Module Federation Runtime的公共依赖 `runtimeLib` ，以及公共的框架依赖`frameworkLib`。[插件详情](/plugin/tool/plugin-rspack-emp-share)
...
:::tip
`@empjs/share/runtime` 提供了远程模块运行时注册的能力，所以在 `emp.config.ts` 中无需再使用 `module federation plugin` 。
:::